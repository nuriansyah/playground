...
func ConvertSecondToTimeStringCorrect(second int) string {
	...
	if hours < 10 && minutes < 10 && seconds < 10 {
		timeString := fmt.Sprintf("0%d:0%d:0%d", hours, minutes, seconds)
		return timeString
	} else if hours < 10 && minutes < 10 {
		timeString := fmt.Sprintf("0%d:0%d:%d", hours, minutes, seconds)
		return timeString
	} else if hours < 10 {
		timeString := fmt.Sprintf("0%d:%d:0%d", hours, minutes, seconds)
		return timeString
	} else if minutes < 10 {
		timeString := fmt.Sprintf("%d:0%d:0%d", hours, minutes, seconds)
		return timeString
	} else if seconds < 10 {
		timeString := fmt.Sprintf("%d:%d:0%d", hours, minutes, seconds)
		return timeString
	} else {
